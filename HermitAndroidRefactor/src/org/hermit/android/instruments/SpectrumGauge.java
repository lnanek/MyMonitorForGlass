    ...
    extends Gauge
{
	...
	private void linearGraph(float[] data, Canvas canvas, Paint paint) {
		...
		CalmVoiceAnalyzer.getInstance().setData(data);
		
        paint.setStyle(Style.FILL);
        paintColor[1] = 1f;
        paintColor[2] = 1f;
        final int len = data.length;
        final float barWidth = (float) (spectGraphWidth - 2) / (float) len;
        final float barHeight = spectGraphHeight - 2;
        final float barBottom = spectGraphY + spectGraphHeight - 1;
        ...
        for (int i = 1; i < len; ++i) {
            ...
            // Draw the bar.
            float barStartX = spectGraphX + 1;
            if (logFreqScale) {
                // For freq f, calculate x.
           		barStartX += (float) (spectGraphWidth*log2(i*lf)/log2(rf) );
            } else {
            	barStartX += i * barWidth;
            }

            
            float barStartY = barBottom - (float) (Math.log10(data[i]) / RANGE_BELS + 1f) * barHeight;
            if (barStartY > barBottom)
                barStartY = barBottom;
            else if (barStartY < spectGraphY)
                barStartY = spectGraphY;
            
            if (barWidth <= 1.0f)
                canvas.drawLine(barStartX, barStartY, barStartX, barBottom, paint);
            else
                canvas.drawRect(barStartX, barStartY, barStartX + barWidth, barBottom, paint);
        }
	}
